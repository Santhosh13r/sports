function SGPopup(config) {
  ...
  var overlayDiv = null;
  var defaultZIndex = config.contentBox.zIndex || 9999;
  ...
  var closeButton = config.closeButton || {
    data: sgpbPublicUrl + "img/close.png",
    width: 16,
    height: 16,
    widthType: "px",
    heightType: "px",
  ...
  var contentBox = config.contentBox || {
    ...
    borderRadiusType: config.contentBox.borderRadiusType,
    borderWidth: 1,
    shadowColor: "#ccc",
    shadowSpread: 0,
    shadowBlur: 10,
    ...
    if (inline) {
      ...
      if (SGPopup.inlinePrevTarget.hasPreviousElement) {
        if (typeof SGPopup.inlinePrevTarget.parentNode !== "undefined") {
          SGPopup.inlinePrevTarget.parentNode.insertBefore(
            htmlElement,
            SGPopup.inlinePrevTarget.node.nextSibling
          );
        }
      } else {
  ...
  function setMainDivStyles(sizeConfig) {
    ...
    if (sizeConfig.minWidth != -1) {
      ...
      if (typeof minWidth === "string" && minWidth.indexOf("%") !== -1) {
        popupMinhWidth = (popupMinhWidth / 100) * parseInt(minWidth);
    ...
    if (sizeConfig.maxHeight != -1 && sizeConfig.maxHeight) {
      ...
      if (maxHeight.indexOf("%") !== -1) {
        popupMaxHeight = (popupMaxHeight / 100) * parseInt(maxHeight);
    ...
    if (sizeConfig.maxWidth != -1 && sizeConfig.maxWidth) {
      ...
      if (maxWidth.indexOf("%") !== -1) {
        popupMaxWidth = (popupMaxWidth / 100) * parseInt(maxWidth);
    ...
    if (contentBox.borderRadius) {
      ...
      if (contentBox.borderRadiusType) {
        borderRadiusMeasure = contentBox.borderRadiusType;
      }
      mainDiv.style.borderRadius =
        contentBox.borderRadius + borderRadiusMeasure;
    }
    ...
    if (widthToSet.indexOf("%") > -1) {
      ...
      if (
        closeBehavior.showButton === false ||
        config.closeButton.type === "button"
      ) {
        closeButtonWidthToSubtract = 0;
      }
      widthToSet =
        (widthNum / 100) * window.innerWidth -
        2 * (contentBox.padding ? contentBox.padding : 0) -
        parseFloat(mainDiv.style.borderLeftWidth) -
        parseFloat(mainDiv.style.borderRightWidth) -
        parseFloat(contentBox.shadowSpread) / 2 -
        closeButtonWidthToSubtract +
        "px";
    } else {
      widthToSet =
        parseFloat(widthToSet) -
        2 * (contentBox.padding ? contentBox.padding : 0) +
        "px";
    }
    ...
    if (contentBox.showBackground) {
      if (contentBox.backgroundImage) {
        mainDiv.style.backgroundImage =
          "url(" + contentBox.backgroundImage + ")";
      }
      if (contentBox.backgroundMode === "cover") {
        ...
        if (!fitBackgroundImg) {
          ...
          fitBackgroundImg.className =
            "sgpb-background-image-" + config.popupId;
  ...
  function resizeConfig(backgroundImage) {
    ...
    if (heightDifference > widthDifference) {
      ...
      var modifiedWidthPercent = Math.floor(
        (widthDifference / imageWidth) * 100
      );
      var heightMustDecrease = Math.floor(
        (imageHeight * modifiedWidthPercent) / 100
      );
    ...
    if (
      ...
      if (position.top === "center") {
        contentDiv.style.top =
          (window.innerHeight - parseInt(mainDiv.clientHeight) - 2 * border) /
            2 +
          "px";
      } else {
  ...
  function setCloseButton(mainDiv) {
    ...
    closeButtonImage.alt = SGPB_JS_LOCALIZATION.closeButtonAltText;
    closeButtonImage.title = SGPB_JS_LOCALIZATION.closeButtonAltText;
  ...
  function positionCloseButton(mainDiv) {
    if (closeBehavior.buttonPosition == "left") {
      ...
      left =
        ...
        2 * contentBox.padding +
        2 * border;
      if (closeBehavior.leftPosition) {
        left -= parseFloat(closeBehavior.leftPosition);
      }
  ...
  function setCloseAnimation() {
    ...
    if (closeAnimation.type == "slideleft") {
      ...
      contentDiv.className =
        prefix +
        "-popup-dialog-main-div-theme-wrapper-" +
        config.popupTheme +
        " sg-animated " +
        closeAnimation.type;
    }
    ...
    window.setTimeout(function () {
      contentDiv.className =
        prefix + "-popup-dialog-main-div-theme-wrapper-" + config.popupTheme;
  ...
  function initPopup() {
    ...
    if (contents) {
      ...
      divElement.setAttribute(
        ...
        "height:100%;width:100%;overflow:" +
          (contentBox.scrollingEnabled ? "auto" : "hidden") +
          ";"
      );
      ...
      mainDiv.innerHTML =
        '<div style="height:100%;width:100%;overflow:' +
        (contentBox.scrollingEnabled ? "auto" : "hidden") +
        ';">' +
    ...
    if (
      typeof config.closeButton.type !== "undefined" &&
      config.closeButton.type == "button"
    ) {
    ...
    if (config.closeBehavior.showButton !== false) {
      closeButtonImage.className =
        prefix + "-popup-close-button-" + config.popupTheme;
...
SGPopup.setCookie = function (cname, cvalue, exdays) {
  ...
  var value =
    ...
    (exdays == null
      ? ";"
      : "; expires=" + exdate.toUTCString() + "; SameSite=" + sameSite);
...
SGPopup.openSGPopup = function () {
  ...
  for (var i = 0; i < ids.length; i++) {
    SGPopup.sendGetRequest(
      SG_APP_URL + "api/popups/" + ids[i],
      responseFunction,
      ids[i]
    );
  }
