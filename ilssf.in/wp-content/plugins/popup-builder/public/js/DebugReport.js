function debugModeInit() {
  ...
  console.log(
    "%c POPUP BUILDER AVAILABLE DEBUG DETAILS",
    ...
    for (var i in debugParamas) {
      ...
      for (var x in paramName) {
        if (typeof paramName[x]["name"] != "undefined") {
          console.log(
            "%c Option name ------- " + paramName[x]["name"],
            "background: #eeeeee; color: #000000"
          );
        }
        if (typeof paramName[x]["operator"] != "undefined") {
          console.log(
            "%c Option operator --- " + paramName[x]["operator"],
            "background: #eeeeee; color: #000000"
          );
        }
        if (typeof paramName[x]["value"] != "undefined") {
          if (typeof paramName[x]["value"] == "string") {
            ...
            console.log(
              "%c Option value ------ " + paramName[x]["value"],
              "background: #eeeeee; color: #000000"
            );
          } else {
            console.log(
              "%c Option value ------ " + Object.values(paramName[x]["value"]),
              "background: #eeeeee; color: #000000"
            );
          }
        }
