...
PopupConfig.firstToLowerCase = function (string) {
  return string[0].toLowerCase() + string.slice(1);
};
PopupConfig.prototype.magicCall = function (functionName, arrgname) {
  if (functionName.indexOf("get") != -1) {
    var param = functionName.replace(/^get/g, "");
    var param = PopupConfig.firstToLowerCase(functionName.replace(/^get/g, ""));
    return this[param];
  } else {
    var param = PopupConfig.firstToLowerCase(functionName.replace(/^set/g, ""));
    this[param] = arrgname;
  }
};
PopupConfig.prototype.isIE = function () {
...
PopupConfig.prototype.combineConfigObj = function () {
  ...
  var config = {
    ...
    sizingRanges: [
      {
        screenFrom: { width: 0, height: 0 },
        screenTo: { width: 300, height: 3000 },
        width: "100%",
        height: this.height,
      ...
      {
        screenFrom: { width: 300, height: 0 },
        screenTo: { width: 5000, height: 3000 },
        width: this.width,
        height: this.height,
        maxWidth: this.maxWidth,
        maxHeight: this.maxHeight,
    ...
    didOpen: function (args) {
      if (that.isIE()) {
        var finalArgs = [args, eventSendArgs],
          finalArgs = finalArgs.reduce(function (r, o) {
